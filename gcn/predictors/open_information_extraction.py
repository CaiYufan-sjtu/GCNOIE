...
class OpenIePredictor(Predictor):
    ...
    def traverse_predicted_heads(self, adj, predicted_heads, word_index):
        if word_index not in adj:
            adj[word_index] = []
        for i in range(len(predicted_heads)):
            if predicted_heads[i] == word_index:
                adj[word_index].append(i + 1)
                if i + 1 in predicted_heads:
                    self.traverse_predicted_heads(adj, predicted_heads, i + 1)
    ...
    def _json_to_instance(self, json_dict: JsonDict) -> Instance:
        ...
        ##########################
        result = self.dependency_tree_predictor.predict(str(tokens).strip())
        predicted_heads = result["predicted_heads"]
        #########################
        verb_index = predicate_index
        #############################################
        adj = {}
        ...
        return self._dataset_reader.text_to_instance(tokens, verb_labels, adj)
